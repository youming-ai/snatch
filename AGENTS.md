...
- **Signed downloads**: `/api/resolve` builds each choice's `/api/download/progress` URL absolute to the API origin and HMAC-signs the params (`lib/security.ts`). The progress endpoint runs yt-dlp, then signs a `/api/download?file=...` URL for the prepared file. The signature carries no expiry of its own — `verifyUrl` checks only the HMAC — so the practical lifetime of the link is whatever the temp file's is: `/api/download` deletes it once a response has carried the whole file (byte 0 to EOF) to a client that stayed connected, and the sweeper reclaims anything else on its TTL. Cross-origin downloads need no CORS because they are an `<a download>` navigation, not a `fetch`. Only `POST /api/resolve` is a cross-origin `fetch`, gated by `ALLOWED_ORIGINS`.
