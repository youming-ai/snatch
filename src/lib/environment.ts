...
export async function getDownloadService() {
	...
	if (env.isVercel) {
		...
		try {
			const { vercelDownloadService } = await import(
				"../services/vercel-download.service"
			);
			return vercelDownloadService;
		} catch (error) {
			console.warn(
				"Failed to load Vercel service, falling back to client service:",
				error,
			);
			const { clientDownloadService } = await import(
				"../services/client-download.service"
			);
		...
		const { downloadService } = await import(
			"../services/unified-download.service"
		);
