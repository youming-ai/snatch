...
export class VercelAdapter {
  ...
  async download(url: string): Promise<DownloadResult[]> {
    ...
    try {
      ...
      console.log(`✅ [Vercel-${this.platform}] Found ${results.length} results`);
      return results;
    } catch (error) {
      console.error(`❌ [Vercel-${this.platform}] Error:`, error);
      throw new Error(`Failed to download from ${this.name}: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
   ...
   */
  private parseOEmbedResponse(data: any, originalUrl: string): DownloadResult[] {
   ...
   */
  private async extractMetadata(url: string, contentId: string): Promise<DownloadResult[]> {
    try {
 ...
 */
export function createVercelAdapter(platform: SupportedPlatform): VercelAdapter {
  switch (platform) {
    case 'instagram':
