...
export class VercelDownloadService {
  ...
  detectPlatform(url: string): SupportedPlatform | null {
    ...
    if (lowercaseUrl.includes('x.com') || lowercaseUrl.includes('twitter.com')) return 'twitter';
  ...
  async download(url: string): Promise<DownloadResponse> {
    try {
      ...
      if (!platform) {
        return {
          success: false,
          error: "Unsupported platform. Please enter Instagram, TikTok, or X (Twitter) URL",
        };
      }
      ...
      console.log(`✅ [Vercel] Download completed for ${platform}:`, results.length, 'items');
