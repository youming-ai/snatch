...
import type { DownloadFormat, DownloadResult as DownloadResultType } from "@/types/download";
...
export function DownloadResult({ result }: DownloadResultProps) {
	...
	const downloadFormat = (format: DownloadFormat) => {
		window.open(format.downloadUrl, "_blank");
	};

	const topQuality = result.formats[0]?.qualityCategory;
	...
	return (
		<div className="group relative bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-purple-500/30 transition-all duration-300">
			...
			<div className="p-4 space-y-3">
				...
				{/* Quality picker — one button per format */}
				<div className="flex flex-col gap-2 pt-2">
					{result.formats.map((format) => (
						<button
							...
							className="w-full px-4 py-2 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-white text-sm font-medium rounded-xl transition-all duration-300 flex items-center justify-between gap-2"
						>
							...
							{format.size && (
								<span className="text-xs text-white/80 font-normal">{format.size}</span>
							)}
						</button>
					))}
					<a
						...
						className="w-full px-4 py-2 bg-white/5 hover:bg-white/10 text-white text-sm font-medium rounded-xl transition-all duration-300 flex items-center justify-center gap-2"
					>
